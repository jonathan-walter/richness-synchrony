# =============================================================================================
# SPATIAL SYNCHRONY FORMATTING AND ANALYSIS FOR HAYES, KS DATA
# =============================================================================================
# ---------------------------------------------------------------------------------------------
# by Nina Lany, University of Virginia, castorani@virginia.edu
...
ggsave(file = here("figures/supplement/hay_richness_over_time.pdf"), width = 7, height = 4.7, units = "in")
...
ggsave(file = here("figures/supplement/hay_sp_acc_curve.pdf"), width = 7, height = 4.7, units = "in")
...
pdf(file=here("figures/supplement/hay_sp_acc_space.pdf"), width = 7, height = 4.7)
plot(as.numeric(no.taxa.space$site), no.taxa.space$no.taxa, pch = 19, xaxt="n", bty="l", xlab = "Cumulative number of sites", ylab = "Cumulative number of taxa", cex=1.5, lwd=3, cex.lab=1.5)
...
mtdt$taxa <- "mixed grass prairie"
mtdt$abund.type <- "percent cover"
mtdt$abund.units <- dat.summary$abund.units
mtdt$extent <- 0.0521
mtdt$interplot.dist <- 0.0261
mtdt <- data.frame(mtdt)

#write metadata
write.csv(mtdt, file = here("empirical/site_summary_table/hay_metadata.csv"), row.names=F)

#write L2 data
#write.csv(dat, file = "GeogSynch/L2_data/hay.csv", row.names=F)


## Running analyses #######################################################
hay_results<-CommSpatSynch(inarray=data_array, do.Modularity=F, do.Mantel=F)
hay_results$n.spp <- dim(hay_data_array)[1]

write_rds(hay_results, file=here("empirical/analyses_by_site/output/hay_results.rds"))
