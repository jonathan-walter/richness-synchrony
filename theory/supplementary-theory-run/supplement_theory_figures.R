...
## this script closely matches manuscriptFigures.R, but is used to make the analogous
## figures where dispersal occurs after growth
...
sup.thry.dat<-read.csv(here("theory/supplementary-theory-run/post_disp_theory_output.csv"))
...
pdf(here("figures/supplement/FigS1_richsynch_combined.pdf"), width=6.5, height=3.25)

layout(matrix(c(1,2),nrow=1),widths=c(0.45,0.55))

par(mar=c(1.5,3.7,1.5,1.1), oma=c(1.3,0,0,0), tcl=-0.4, mgp=c(3,0.5,0))

hist(sup.thry.dat$rRichness, breaks=seq(-.1,1,by=.1), freq=TRUE, xlim=c(-2/20,1), 
...
thry.std<-sup.thry.dat[,colnames(sup.thry.dat) %in% 
...
pdf(here("figures/supplement/FigS2_thry_effects.pdf"), width=3.25, height=3.25)
...
##--------------------------------------------------------------------------
## Figure 3: community cv ~ richness synchrony and avg richness

ptcol<-c(bb,bg,bg,bg,bg,bg,bg,bg,bb,bb,bb,bb,bb,bb,bb,bg,bg,bg,bb,bb)

fit3<-lm(sup.thry.dat$cv~sup.thry.dat$rRichness)
cor.test(sup.thry.dat$cv,sup.thry.dat$rRichness)

fit4<-lm(emp.dat$CVTotBiomass~emp.dat$rRichness)
cor.test(emp.dat$CVTotBiomass,emp.dat$rRichness)

cor.test(sup.thry.dat$cv,sup.thry.dat$AvgPlotRich)
fit5<-lm(sup.thry.dat$cv~sup.thry.dat$AvgPlotRich)

fit6<-lm(emp.dat$CVTotBiomass~emp.dat$AvgPlotRich)
cor.test(emp.dat$CVTotBiomass,emp.dat$AvgPlotRich)

pdf(here("figures/supplement/FigS3_cv_richsynch.pdf"), width=6.5, height=6.5)

par(mar=c(3.1,3.1,1.5,0.5), tcl=-0.4, mgp=c(1.75,0.5,0), mfcol=c(2,2), oma=c(0,0,0,0.6))

plot(sup.thry.dat$rRichness, sup.thry.dat$cv, xlab="Richness synchrony", ylab="Community CV", xlim=c(0,1), ylim=c(0,1),
     pch=20, col="grey")
abline(fit3, lwd=2)
...
plot(sup.thry.dat$AvgPlotRich, sup.thry.dat$cv, xlab="Richness", ylab="Community CV", ylim=c(0,1), pch=20, col="grey")
abline(fit5,lwd=2)
...
# fit3a<-lm(sup.thry.dat$cv~scale(sup.thry.dat$rRichness))
# fit5a<-lm(sup.thry.dat$cv~scale(sup.thry.dat$AvgPlotRich))
# fit4a<-lm(emp.dat$CVTotBiomass~scale(emp.dat$rRichness))
# fit6a<-lm(emp.dat$CVTotBiomass~scale(emp.dat$AvgPlotRich))


summary(lm(sup.thry.dat$cv~sup.thry.dat$rRichness))
summary(lm(sup.thry.dat$cv~sup.thry.dat$AvgPlotRich))
summary(lm(sup.thry.dat$cv~sup.thry.dat$Evenness))
summary(lm(sup.thry.dat$cv~sup.thry.dat$Turnover))
summary(lm(sup.thry.dat$cv~sup.thry.dat$Jaccard))
