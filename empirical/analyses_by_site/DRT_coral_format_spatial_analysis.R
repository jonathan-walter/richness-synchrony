# =============================================================================================
# SPATIAL SYNCHRONY FORMATTING AND ANALYSIS FOR DRY TORTUGAS CORALS DATA
# =============================================================================================
# ---------------------------------------------------------------------------------------------
...
dat<-dat[dat$plot %in% c("Bird_Key_Reef","Black_Coral_Rock","Loggerhead_Patch","Mayers_Peak",
                         "Palmata_Patch","Prolifera_Patch","Temptation_Rock","White_Shoal"),]

# ---------------------------------------------------------------------------------------------
# Clone specific data sets with a generic name
dat.name <- 'drt_coral'
dat.site <- 'dry_tortugas'
dat.domain <- 'marine'
...
mtdt$extent <- 16.514#dat.summary$dist.max
mtdt$interplot.dist <- 6.725#mean(dat.summary$dist.mat[lower.tri(dat.summary$dist.mat, diag=F)])
mtdt <- data.frame(mtdt)
#write metadata
write.csv(mtdt, file = here("empirical/site_summary_table",paste(dat.name, "_metadata.csv", sep ="")), row.names=F)

#write L2 data
#write.csv(dat, file = "GeogSynch/L2_data/drt_coral.csv", row.names=F)

# ---------------------------------------------------------------------------------------------
# Run synchrony analyses 
# ---------------------------------------------------------------------------------------------
drt_coral_results <- CommSpatSynch(inarray = data_array, do.Modularity = F, do.Mantel=F)
