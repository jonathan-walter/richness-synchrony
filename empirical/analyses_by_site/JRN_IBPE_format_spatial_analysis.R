# =============================================================================================
# SPATIAL SYNCHRONY FORMATTING AND ANALYSIS FOR JORNADA (JRN - IBPE) DATA
# =============================================================================================
# ---------------------------------------------------------------------------------------------
# Revised by Max Castorani, University of Virginia, castorani@virginia.edu
# Revised on 2018-05-10
# ---------------------------------------------------------------------------------------------
rm(list=ls())
# ---------------------------------------------------------------------------------------------
...
# Select data
# ---------------------------------------------------------------------------------------------
dat<-grasslands[grasslands$site=="jrn",]
dat<-dat[dat$project=="IBPE",]
...
no.taxa$no.taxa <-  left_join(no.taxa$no.taxa, 
                              dat[dat$year == min(dat$year), c('site', 'habitat', 'plot', 'subplot', 'project', 'uniqueID')], 
                              by = 'uniqueID') %>%
  dplyr::select(project, uniqueID, site, habitat, plot, subplot, year, no.taxa)
...
pdf(file=here("figures/supplement/jrn_ibpe_sp_acc_space.pdf"), width = 7, height = 4.7)
plot(sites, no.taxa.space$no.taxa, pch = 19, xaxt="n", bty="l", xlab = "Cumulative number of sites", 
     ylab = "Cumulative number of taxa", cex=1.5, lwd=3, cex.lab=1.5)
...
mtdt$extent <- 0.0846
mtdt$interplot.dist <- 0.0368
mtdt <- data.frame(mtdt)
#write metadata
write.csv(mtdt, file = here("empirical/site_summary_table/",paste(dat.name, "_metadata.csv", sep ="")), row.names=F)
#write L2 data
#write.csv(dat, file = "GeogSynch/L2_data/jrn_ibpe.csv", row.names=F)

# ---------------------------------------------------------------------------------------------
# Run synchrony analyses 
# ---------------------------------------------------------------------------------------------

jrn_ibpe_results <- CommSpatSynch(inarray=data_array, do.Modularity=FALSE, do.Mantel=FALSE)
