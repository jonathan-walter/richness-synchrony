...
ggsave(file = here("figures/supplement/sev_c_richness_over_time.pdf"), width = 7, height = 4.7, units = "in")# Note that the thick line indicates the total number of taxa among all subplots
...
ggsave(file = here("figures/supplement/sev_c_sp_acc_curve.pdf"), width = 7, height = 4.7, units = "in")
...
mtdt$extent <-  1.327 
mtdt$interplot.dist <- 0.501
mtdt <- data.frame(mtdt)
#write metadata
write.csv(mtdt, file = here("empirical/site_summary_table/sev_c_metadata.csv"), row.names=F)
...
sev_c_results<-CommSpatSynch(inarray=data_array, do.Mantel=FALSE, do.Modularity=FALSE)
sev_c_results$n.spp <- dim(data_array)[1]

saveRDS(sev_c_results, file=here("empirical/analyses_by_site/output/sev_c_results.rds"))
