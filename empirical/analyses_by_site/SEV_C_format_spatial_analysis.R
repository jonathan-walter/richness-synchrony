...
# -------------------------------------------------------------------------
# Add SEV coordinates manually ##<-these still need to be added if possible################
...
sev_coords <-subset(sev_coords, SiteCode=="C")
...
ggsave(file = 'GeogSynch/Manuscripts/1_Data_supplemental_methods/sev_c_richness_over_time.pdf', width = 7, height = 4.7, units = "in")# Note that the thick line indicates the total number of taxa among all subplots
...
ggsave(file = 'GeogSynch/Manuscripts/1_Data_supplemental_methods/sev_c_sp_acc_curve.pdf', width = 7, height = 4.7, units = "in")
...
#write metadata
write.csv(mtdt, file = "GeogSynch/Scripts/make_site_table/site_summaries/sev_c_metadata.csv", row.names=F)

#write L2 data
write.csv(dat, file = "GeogSynch/L2_data/sev_c.csv", row.names=F)
...
sev_c_results<-sev_results

save(sev_c_results, file="GeogSynch/Scripts/format_L2_data/Output/sev_c_results.RData")
