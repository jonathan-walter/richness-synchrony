...
inUrl1  <- "https://pasta.lternet.edu/package/data/eml/knb-lter-sbc/50/7/24d18d9ebe4f6e8b94e222840096963c" 
infile1 <- tempfile()
download.file(inUrl1,infile1,method="curl")

sbc <-read.csv(infile1,header=F 
               ,skip=1
               ,sep=","  
               , col.names=c(
                 "YEAR",     
                 ...
                 "MOBILITY",     
                 "GROWTH_MORPH",     
                 "COARSE_GROUPING"    ), check.names=TRUE, stringsAsFactors=FALSE)
...
sbc_results <- CommSpatSynch(inarray = data_array, do.Modularity=F, do.Mantel=F)
sbc_results$n.spp <- dim(data_array)[1]

saveRDS(sbc_results, file = here("empirical/analyses_by_site/output",paste(dat.name, "_results.rds", sep ="")))
