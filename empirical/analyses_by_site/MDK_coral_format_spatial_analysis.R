# =============================================================================================
# SPATIAL SYNCHRONY FORMATTING AND ANALYSIS FOR MIDDLE KEYS CORALS DATA
# =============================================================================================
# ---------------------------------------------------------------------------------------------
# by Max Castorani, University of Virginia, castorani@virginia.edu
...
dat.name <- 'mdk_coral'
dat.site <- 'middle_keys'
#dat.habitat <- ###
dat.domain <- 'marine'

dat <- florida.data_clean[florida.data_clean$plot %in% coral.geo.data$plot[coral.geo.data$sub.location=="Middle_Keys"],]; rm(florida.data_clean)

coords <- coral.geo.data[coral.geo.data$sub.location=="Middle_Keys",]; rm(coral.geo.data)
...
#write L2 data
write.csv(dat, file = "GeogSynch/L2_data/mdk_coral.csv", row.names=F)
...
mdk_coral_results <- CommSpatSynch(inarray = data_array, do.Modularity = F, do.Mantel=F)

mdk_coral_results$n.spp <- dim(data_array)[1]

save(mdk_coral_results, file = paste("GeogSynch/Scripts/format_L2_data/Output/", dat.name, "_results.RData", sep =""))
