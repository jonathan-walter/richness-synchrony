# =============================================================================================
# SPATIAL SYNCHRONY FORMATTING AND ANALYSIS FOR JORNADA (JRN - SUMM) DATA
# =============================================================================================
# ---------------------------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------------------------
#make entry for table with site (community) level variables:
mtdt <- list()
...
mtdt$extent <- 0.0869
mtdt$interplot.dist <- 0.0372
mtdt <- data.frame(mtdt)
#write metadata
write.csv(mtdt, file = here("empirical/site_summary_table/",paste(dat.name, "_metadata.csv", sep ="")), row.names=F)

#write L2 data 
#write.csv(dat, file = "GeogSynch/L2_data/jrn_summ.csv", row.names=F)
# 

# ---------------------------------------------------------------------------------------------
# Run synchrony analyses 
# ---------------------------------------------------------------------------------------------
jrn_summ_results <- CommSpatSynch(inarray = data_array, do.Modularity=F, do.Mantel=F)
