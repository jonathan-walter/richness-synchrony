...
sev_coords <-subset(sev_coords, SiteCode=="B")
...
ggsave(file = 'GeogSynch/Manuscripts/1_Data_supplemental_methods/sev_b_richness_over_time.pdf', width = 7, height = 4.7, units = "in")# Note that the thick line indicates the total number of taxa among all subplots
...
ggsave(file = 'GeogSynch/Manuscripts/1_Data_supplemental_methods/sev_b_sp_acc_curve.pdf', width = 7, height = 4.7, units = "in")
...
mtdt$interplot.dist <-  mean(dat.summary$dist.mat)
mtdt <- data.frame(mtdt)
#write metadata
write.csv(mtdt, file = "GeogSynch/Scripts/make_site_table/site_summaries/sev_b_metadata.csv", row.names=F)

#write L2 data
write.csv(dat, file = "GeogSynch/L2_data/sev_b.csv", row.names=F)
...
#sev_results <- Reumannplatz:::CommSpatSynch(inarray=sev_data_array, patch.het=sev_habitats)
sev_results<-CommSpatSynch(inarray=sev_data_array, patch.het=NULL)
#sev_results$n.habitats <- length(unique(sev_habitats))
sev_results$n.spp <- dim(sev_data_array)[1]
sev_b_results<-sev_results

save(sev_b_results, file="GeogSynch/Scripts/format_L2_data/Output/sev_b_results.RData")
