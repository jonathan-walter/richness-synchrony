...
ggsave(file = here("figures/supplement/sev_b_richness_over_time.pdf"), width = 7, height = 4.7, units = "in")# Note that the thick line indicates the total number of taxa among all subplots
...
ggsave(file = here("figures/supplement/sev_b_sp_acc_curve.pdf"), width = 7, height = 4.7, units = "in")
...
mtdt$extent <-  0.704 
mtdt$interplot.dist <-  0.312
mtdt <- data.frame(mtdt)
#write metadata
write.csv(mtdt, file = here("empirical/site_summary_table/sev_b_metadata.csv"), row.names=F)
...
sev_b_results<-CommSpatSynch(inarray=data_array, do.Mantel=FALSE, do.Modularity=FALSE)
sev_b_results$n.spp <- dim(data_array)[1]

saveRDS(sev_b_results, file=here("empirical/analyses_by_site/output/sev_b_results.rds"))
