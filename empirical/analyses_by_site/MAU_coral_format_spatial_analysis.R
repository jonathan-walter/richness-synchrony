...
# Select data
# ---------------------------------------------------------------------------------------------
dat<-marine[marine$site=="Main_Hawaiian_Islands",]
dat<-dat[dat$year >=2001,] #some sites have missing data before 2001
...
mtdt$extent <- 50.411
mtdt$interplot.dist <- 23.871
mtdt <- data.frame(mtdt)
#write metadata
write.csv(mtdt, file = here("empirical/site_summary_table",paste(dat.name, "_metadata.csv", sep ="")), row.names=F)

#write L2 data
#write.csv(dat, file = "GeogSynch/L2_data/mau_coral.csv", row.names=F)


# ---------------------------------------------------------------------------------------------
# Run synchrony analyses 
# ---------------------------------------------------------------------------------------------
mau_coral_results <- CommSpatSynch(inarray = data_array, do.Modularity = F, do.Mantel=F)
